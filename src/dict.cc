...
#include "dict.h"
  ...
  void dict_insert(unsigned long id, const char* key, const char* value) {
    ...
    if(DEBUG) {
      std::cerr << "dict_insert(" << id << ", "<< format_cstring(key) << ", " << format_cstring(value) << ")\n";
    }
    ...
    if(DEBUG) {
      std::cerr << "dict_insert: dict " << id << ", the pair (" << format_cstring(key) << ", " << format_cstring(value) << ") has been inserted\n";
    }
  }
  ...
  const char* dict_find(unsigned long id, const char* key) {
    ...
    if(i == dictionaries[id].end()) {
      ...
      if(DEBUG) {
        std::cerr << "dict_find: dict " << id << ", the key \"" << (i->first) << "\" has the value \"" << (i->second) << "\"\n";
      }
      return value;
    }
  }
